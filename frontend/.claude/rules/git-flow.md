...
- **`master`** — 배포 기준 브랜치.
- **`dev-fe`** — 프론트엔드 개발 통합 브랜치. 작업 브랜치는 여기서 분기하고, 작업이 끝나면 다시 `dev-fe`로 PR을 올린다.
- 작업 브랜치를 새로 만들 때는 `dev-fe`를 기준으로 분기한다.
- **`master`로의 PR은 `dev-fe`에서만 연다.** `feature/`·`fix/`·`docs/`·`refactor/`·`chore/` 등 작업 브랜치는 `master`로 직접 PR을 열지 않는다 — 항상 `dev-fe`로 PR을 올리고, `dev-fe`가 쌓인 뒤 `dev-fe → master` PR로 통합한다. (`.github/workflows/pr-guard.yml`이 `master`로 향하는 PR의 head 브랜치가 `dev-fe`가 아니면 자동으로 실패시킨다.)
...
- **Require status checks to pass before merging** — `Frontend CI`와 `PR Target Guard`를 필수 체크로 지정한다.
